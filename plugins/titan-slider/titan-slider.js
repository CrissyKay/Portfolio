(function ($) {
  ...
  if (slide.eq(0).hasClass("dark")) $("html").addClass("dark");
  ...
  slider.on("mousemove", function (e) {
    ...
    if (mousePos.x > s.width() / 2) {
      s.find(".slides").css(
        "cursor",
        "url(/plugins/titan-slider/slide-right-dark.png) 5 -25, e-resize"
      );
      if ($("html").hasClass("dark"))
        s.find(".slides").css(
          "cursor",
          "url(/plugins/titan-slider/slide-right.png) 5 -25, e-resize"
        );
      s.addClass("next").removeClass("previous");
    } else {
      s.find(".slides").css(
        "cursor",
        "url(/plugins/titan-slider/slide-left-dark.png) 5 -25, w-resize"
      );
      if ($("html").hasClass("dark"))
        s.find(".slides").css(
          "cursor",
          "url(/plugins/titan-slider/slide-left.png) 5 -25, w-resize"
        );
  ...
  slider.on("click", function () {
    ...
    TweenMax.to(slides, 1, {
      left: "+=" + -sliderWidth * delta,
      ease: "easeInOutQuart",
      onComplete: function () {
        slide.eq(current).find("video").trigger("pause");

        if (cycle) {
          TweenMax.set(slides, { left: -sliderWidth * current });
        }
        ...
        if (slide.eq(current).hasClass("dark")) $("html").addClass("dark");
  ...
  $(window).on("resize", function () {
    slider.each(function (e) {
      ...
      thisSlides.css({
        left: -thisSliderWidth * curSlide,
        width: thisSlidesWidth,
      });
    });
  });
})(jQuery);
